...
async function getPortalDataForUser(currentUser, sessionToken = "") {
  ...
  const [paymentMethods, workLogs, payments, escrows, deposits, contracts, posts, bidProfiles, disputes, notifications, chatContacts, chatMessages, supportContacts, supportMessages] = await Promise.all([
    portal.paymentMethods.find(paymentMethodFilter, { projection: { _id: 0 } }).sort({ isPrimary: -1, updatedAt: -1 }).toArray(),
    portal.workLogs.find(userFilter, { projection: { _id: 0 } }).sort({ workDate: -1, createdAt: -1 }).toArray(),
    portal.payments.find(paymentFilter, { projection: { _id: 0 } }).sort({ scheduledDate: -1, createdAt: -1 }).toArray(),
  ...
  return {
    ...
    payments: payments.map((payment) => paymentForViewer(payment, safeCurrentUser)),
