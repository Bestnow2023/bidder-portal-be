...
async function getPortalDataForUser(currentUser, sessionToken = "") {
  ...
  const [paymentMethods, workLogs, payments, escrows, deposits, contracts, posts, bidProfiles, disputes, notifications, chatContacts, chatMessages, supportContacts, supportMessages] = await Promise.all([
    ...
    portal.posts.find(postFilterForUser(safeCurrentUser), { projection: { _id: 0 } }).sort({ boostedAt: -1, updatedAt: -1, createdAt: -1 }).limit(200).toArray(),
...
export async function updateDispute(email, payload) {
  ...
  await Promise.all([
    createUserNotification(portal, {
      ...
      body: `Super admin changed ${dispute.subject || "a dispute"} to ${status}.`,
      relatedDisputeId: dispute.id,
      actorUserId: currentUser.id,
    }),
    dispute.targetUserId
      ? createUserNotification(portal, {
        recipientUserId: dispute.targetUserId,
        type: "dispute_resolved",
        title: "Dispute status updated",
        body: `Super admin changed ${dispute.subject || "a dispute"} to ${status}.`,
        relatedDisputeId: dispute.id,
...
export async function repostPost(email, payload) {
  const currentUser = await getSessionUser(email, payload.sessionToken);
  if (currentUser.status !== "approved") {
    throw new Error("Only approved users can repost posts.");
  }
...
export async function updateContract(email, payload) {
  ...
  await Promise.all([
    createUserNotification(portal, {
      ...
      body: `${currentUser.name || "Client"} updated ${title}.`,
      relatedContractId: contract.id,
      actorUserId: currentUser.id,
    }),
    createSuperAdminNotification(portal, {
  ...
  if (status === "ended") {
    ...
    await Promise.all([
      createUserNotification(portal, {
        ...
        body: `${currentUser.name || "A member"} ended ${contract.title}.`,
        relatedContractId: contract.id,
        actorUserId: currentUser.id,
      }),
      createSuperAdminNotification(portal, {
        type: "contract_ended",
        title: "Contract ended",
        body: `${currentUser.name || "A member"} ended contract ${contract.id}.`,
        clientId: client.id,
        relatedContractId: contract.id,
        actorUserId: currentUser.id,
      }),
