...
async function ensureMonthlyPostCredit(portal, user, stamp = now()) {
  ...
  const refillAmount = Math.max(0, roundMoney(SIGNUP_POST_CREDIT - currentBalance));
...
async function spendPostingCredit(portal, user, postId, stamp) {
  ...
  const postCreditUsed = Math.min(POST_CREDIT_COST, balances.postCreditBalance);
  const moneyCreditUsed = roundMoney(POST_CREDIT_COST - postCreditUsed);
  if (moneyCreditUsed > balances.moneyCreditBalance) {
    throw new Error("You need at least $1 post credit or money credit to publish a post.");
  }
...
async function notifyBidProfileAssigned(portal, { bidderId, client, profile, actor }) {
  ...
  await createUserNotification(portal, {
    ...
    title: "New bid profile assigned",
    body: `${client?.name || "A client"} sent you the bid profile ${profile.profileName || "Untitled profile"}.`,
...
async function getPortalDataForUser(currentUser, sessionToken = "") {
  ...
  const [paymentMethods, workLogs, payments, escrows, deposits, contracts, posts, bidProfiles, disputes, notifications, chatContacts, chatMessages, supportContacts, supportMessages] = await Promise.all([
    ...
    portal.chatMessages
      .find(supportMessageFilterForUser(safeCurrentUser), { projection: { _id: 0 } })
      .sort({ createdAt: 1 })
      .limit(200)
      .toArray(),
  ]);
  ...
  return {
    ...
    supportContacts,
    supportMessages: stripMongoIds(supportMessages),
  };
}
...
export async function createUserAsAdmin(adminEmail, payload) {
  ...
  const statusInput = cleanText(payload.status, "pending");
  const status = ["pending", "approved", "paused"].includes(statusInput) ? statusInput : "pending";
  const portal = collections(await getDb());
...
async function cleanBidProfileAssignees(portal, clientId, bidderIds) {
  ...
  if (validIds.size !== requestedIds.length) {
    throw new Error("Select approved bidders assigned to this client.");
  }
...
export async function saveBidProfile(email, payload) {
  ...
  await Promise.all(
    assignedBidderIds
      .filter((bidderId) => !existingAssigned.has(bidderId))
...
export async function assignBidProfile(email, payload) {
  ...
  if (!bidder || bidder.role !== "bidder" || bidder.status !== "approved" || bidder.assignedAdminId !== existingProfile.clientId || !targetVisibleInLive(bidder)) {
    throw new Error("Select an approved bidder assigned to this client.");
  }
...
export async function deletePost(email, payload) {
  ...
  const stamp = now();
  await Promise.all([
    portal.posts.deleteOne({ id: post.id }),
    portal.contracts.updateMany({ sourcePostId: post.id }, { $set: { sourcePostId: "", updatedAt: stamp } }),
    portal.chatMessages.updateMany({ relatedPostId: post.id }, { $set: { relatedPostId: "", updatedAt: stamp } }),
  ]);
...
export async function updateContract(email, payload) {
  ...
  const nextPaymentDate = contractNextPaymentDateFromPayload(payload, frequency, weekday, startDate);
  const ratePerApplication = payload.ratePerApplication === undefined
    ? roundMoney(contract.ratePerApplication)
    : roundMoney(payload.ratePerApplication);
  const bonusPerInterview = payload.bonusPerInterview === undefined
    ? roundMoney(contract.bonusPerInterview)
    : roundMoney(payload.bonusPerInterview);
  const stamp = now();
  const updates = {
    title,
    criteria,
...
export async function requestWithdrawal(email, payload) {
  ...
  if (currentUser.status !== "approved") {
    throw new Error("Only approved users can request withdrawals.");
  }
...
export async function addSupportMessage(email, payload) {
  const currentUser = await getSessionUser(email, payload.sessionToken);
  if (currentUser.status !== "approved") {
    throw new Error("Only approved users can send support messages.");
  }
  ...
  if (isSuperAdmin(currentUser)) {
    ...
    if (!recipient || isSuperAdmin(recipient) || recipient.status !== "approved" || !targetVisibleInLive(recipient)) {
...
export async function markChatConversationRead(email, payload) {
  const currentUser = await getSessionUser(email, payload.sessionToken);
  if (currentUser.status !== "approved") {
    throw new Error("Only approved users can read inbox messages.");
  }
  if (isSuperAdmin(currentUser)) {
