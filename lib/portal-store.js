...
const SIGNUP_ROLES = new Set(["admin", "bidder", "developer"]);
...
function canManageChatMessage(user, message) {
  return isAdminRole(user.role) || message.userId === user.id;
}
...
export async function signUp(emailInput, nameInput, passwordInput, roleInput = "bidder") {
  ...
  const user = await createUser(
    email,
    nameInput,
    firstLiveUser ? "admin" : requestedRole,
...
async function visibleUsersForCurrentUser(portal, currentUser) {
  ...
  if (currentUser.role === "admin") {
    const assignedUsers = stripMongoIds(
      await portal.users
        .find(
          {
            ...baseFilter,
    ...
    return [publicUser(currentUser), ...assignedUsers];
  }
...
async function attachUserStats(portal, users) {
  const enrichedUsers = users.map((user) => ({ ...user }));
  const clientIds = enrichedUsers.filter((user) => user.role === "admin").map((user) => user.id);
  ...
  const [workTotals, paymentTotals, escrowTotals] = await Promise.all([
    ...
    statBidderIds.length
      ? portal.payments
          .aggregate([
            { $match: { userId: { $in: statBidderIds }, status: "paid" } },
...
async function chatUserIdsForCurrentUser(portal, currentUser, users) {
  const baseFilter = userVisibilityFilter();
  if (isSuperAdmin(currentUser)) {
    return (
      await portal.users.find(baseFilter, { projection: { _id: 0, id: 1 } }).toArray()
    ).map((user) => user.id);
  }
  ...
  const superAdmins = await portal.users.find({ ...baseFilter, role: "super_admin" }, { projection: { _id: 0, id: 1 } }).toArray();
  superAdmins.forEach((user) => ids.add(user.id));

  if (currentUser.assignedAdminId) {
    ids.add(currentUser.assignedAdminId);
  }
...
async function getPortalDataForUser(currentUser, sessionToken = "") {
  ...
  const users = await attachUserStats(portal, await visibleUsersForCurrentUser(portal, currentUser));
  const recordUserIds = isAdminRole(safeCurrentUser.role)
    ? users.filter(isPortalWorker).map((user) => user.id)
    : [safeCurrentUser.id];
  const userFilter = { userId: { $in: recordUserIds } };
  const chatUserIds = await chatUserIdsForCurrentUser(portal, safeCurrentUser, users);
  ...
  const [paymentMethods, workLogs, payments, escrows, chatMessages] = await Promise.all([
    ...
    portal.chatMessages
      .find({ userId: { $in: chatUserIds } }, { projection: { _id: 0 } })
      .sort({ createdAt: 1 })
      .limit(80)
      .toArray(),
  ]);
...
function canManageUser(actor, target) {
  ...
  return actor.role === "admin" && target.role === "bidder" && target.assignedAdminId === actor.id;
}
...
function canManagePaymentUser(actor, target) {
  ...
  return actor.role === "admin" && target.assignedAdminId === actor.id;
}
...
export async function signIn(emailInput, passwordInput, nameInput) {
  ...
  if (!user) {
    ...
    const isDemoOwner = email === ADMIN_EMAIL;
    user = await createUser(
      email,
      nameInput,
      isDemoOwner ? "super_admin" : "bidder",
      isDemoOwner ? "approved" : "pending",
...
export async function setUserPasswordAsAdmin(adminEmail, payload) {
  ...
  if (!isAdminRole(admin.role)) {
    throw new Error("Only clients and super admins can reset user passwords.");
  }
...
export async function updateUserAsAdmin(adminEmail, payload) {
  const admin = await getSessionUser(adminEmail, payload.sessionToken);
  if (!isAdminRole(admin.role)) {
    throw new Error("Only clients and super admins can manage users.");
  }
  ...
  const safeRole = isSuperAdmin(admin) ? role : target.role;
  const safeStatus = ["pending", "approved", "paused"].includes(status) ? status : target.status;
  const isAdminRoleTarget = isAdminRole(safeRole);
  const portal = collections(await getDb());
  ...
  if (isSuperAdmin(admin)) {
    ...
    if (assignedAdminId) {
      ...
      if (!assignedAdmin || assignedAdmin.role !== "admin" || assignedAdmin.status !== "approved" || !targetVisibleInLive(assignedAdmin)) {
        throw new Error("Select a valid client for this bidder.");
      }
    }
  } else if (safeRole === "bidder") {
  ...
  await portal.users.updateOne(
    { id: target.id },
    {
      $set: {
        ...
        status: safeStatus,
        assignedAdminId,
        ratePerApplication: isAdminRoleTarget ? 0 : cleanNumber(payload.ratePerApplication, target.ratePerApplication),
        bonusPerInterview: isAdminRoleTarget ? 0 : cleanNumber(payload.bonusPerInterview, target.bonusPerInterview),
        nextPaymentDate,
        paymentSchedule,
        paymentFrequency: isAdminRoleTarget ? "" : frequency,
        paymentWeekday: isAdminRoleTarget ? "" : weekday,
        updatedAt: stamp,
      },
    }
  );
  if (target.role === "admin" && safeRole !== "admin") {
...
export async function deleteUserAsAdmin(adminEmail, payload) {
  const admin = await getSessionUser(adminEmail, payload.sessionToken);
  if (!isAdminRole(admin.role)) {
    throw new Error("Only clients and super admins can remove users.");
  }
...
export async function saveProfile(email, payload) {
  ...
  const profileLocation = cleanText(payload.profileLocation).slice(0, 120);
  const profileTimeZone = cleanText(payload.profileTimeZone).slice(0, 80);
  const profileCompletedAt = profileTitle && profileBio && profileLocation
...
export async function addEscrowAsClient(email, payload) {
  ...
  if (!targetClient || targetClient.role !== "admin" || !targetVisibleInLive(targetClient)) {
...
export async function addPaymentAsAdmin(adminEmail, payload) {
  const admin = await getSessionUser(adminEmail, payload.sessionToken);
  if (!isAdminRole(admin.role)) {
    throw new Error("Only clients and super admins can add payments.");
  }
...
export async function editPaymentAsAdmin(adminEmail, payload) {
  const admin = await getSessionUser(adminEmail, payload.sessionToken);
  if (!isAdminRole(admin.role)) {
    throw new Error("Only clients and super admins can edit payments.");
  }
...
export async function deletePaymentAsAdmin(adminEmail, payload) {
  const admin = await getSessionUser(adminEmail, payload.sessionToken);
  if (!isAdminRole(admin.role)) {
    throw new Error("Only clients and super admins can delete payments.");
  }
