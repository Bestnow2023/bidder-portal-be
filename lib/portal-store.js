...
import { withdrawalAvailableAt } from "./payment-hold.js";
import { overduePaymentAmount } from "./overdue-payment.js";
import { approvedClientWorkFilter } from "./payable-work.js";
import { paymentReceivedEmail } from "./payment-email.js";
...
const DEFAULT_UNSAFE_INBOX_TERMS = [
  "fuck",
  "shit",
  "bitch",
  "asshole",
...
const CONTRACT_WORK_DAYS = new Set(["weekdays", "weekends", "all"]);
...
const CHAT_ATTACHMENT_META_PROJECTION = { _id: 0, "attachments.dataUrl": 0 };
const WEEKDAY_INDEX = {
  sunday: 0,
  monday: 1,
  tuesday: 2,
...
function unsafeInboxTerms() {
  const configuredTerms = cleanText(process.env.BLOCKED_INBOX_WORDS || process.env.PORTAL_BLOCKED_WORDS);
  const terms = configuredTerms
    ? configuredTerms.split(",")
    : DEFAULT_UNSAFE_INBOX_TERMS;

  return terms.map((term) => cleanText(term).toLowerCase()).filter(Boolean);
}

function unsafeInboxTermMatches(text, term) {
  const escapedTerm = escapeRegExp(term).replace(/\s+/g, "\\s+");
  if (!escapedTerm) {
    return false;
  }

  return new RegExp(`(^|[^a-z0-9])${escapedTerm}(s|ed|ing)?([^a-z0-9]|$)`, "i").test(text);
}

function assertSafeInboxText(...values) {
  const text = values.map((value) => cleanText(value)).filter(Boolean).join(" ");
  if (!text) {
    return;
  }

  if (unsafeInboxTerms().some((term) => unsafeInboxTermMatches(text, term))) {
    throw new Error("This message contains blocked unsafe words. Edit the message before sending.");
  }
}
...
function stripChatAttachmentDataUrls(messages = []) {
  return messages.map((message) => {
    const attachments = Array.isArray(message.attachments) ? message.attachments : [];
    if (!attachments.length) {
      return message;
...
function chatParticipantIdsFromConversationId(conversationId) {
  return cleanText(conversationId).split("__").map((id) => cleanText(id)).filter(Boolean);
}
...
function normalizeContractWorkDays(value, fallback = "weekdays") {
  const workDays = cleanText(value, fallback).toLowerCase().replace(/[\s-]+/g, "_");
  return CONTRACT_WORK_DAYS.has(workDays) ? workDays : fallback;
}

function contractCoversWorkDate(contract, workDate) {
  ...
  const endedDate = contract.endedAt ? cleanText(contract.endedAt).slice(0, 10) : cleanText(contract.endDate);
  return !endedDate || workDate <= endedDate;
}

function isWeekendWorkDate(workDate) {
...
function assertContractAllowsWorkDate(contract, workDate) {
  const workDays = normalizeContractWorkDays(contract?.workDays);
  if (workDays === "all") {
    return;
  }

  const weekend = isWeekendWorkDate(workDate);
  if (workDays === "weekdays" && weekend) {
    throw new Error("This contract only allows weekday work logs.");
  }
  if (workDays === "weekends" && !weekend) {
    throw new Error("This contract only allows weekend work logs.");
  }
}
...
async function activeContractForWorkLog(portal, userId, contractId, workDate) {
  const filter = {
    workerId: userId,
    status: "active",
    ...(contractId ? { id: contractId } : {}),
  };
  const contracts = stripMongoIds(
    await portal.contracts.find(filter, { projection: { _id: 0 } }).toArray()
  );
  const matchingContract = contracts.find((contract) => contractCoversWorkDate(contract, workDate));

  if (!matchingContract) {
    throw new Error(contractId ? "Selected contract is not active for this work date." : "Select an active contract for this work date.");
  }

  assertContractAllowsWorkDate(matchingContract, workDate);
...
async function fetchPortalPage(collection, filter, sort, resource, options = {}) {
  ...
  const docs = await collection
    .find(filter, { projection: options.projection || { _id: 0 } })
    .sort(sort)
    .skip(offset)
    .limit(limit + 1)
    .toArray();
...
async function initializeSchema() {
  ...
  await Promise.all([
    ...
    portal.workLogs.createIndex({ contractId: 1 }),
...
async function attachUserStats(portal, users, currentUser) {
  ...
  const creditBalancesByUser = await creditBalancesForUsers(
    portal,
    enrichedUsers.filter((user) => canViewCreditBalance(currentUser, user))
  );
...
async function ensureMonthlyPostCredit(portal, user, stamp = now()) {
  await ensureMonthlyPostCreditForUsers(portal, [user], stamp);
}

async function ensureMonthlyPostCreditForUsers(portal, users, stamp = now()) {
  const eligibleUsers = (Array.isArray(users) ? users : [])
    .filter((user) => user?.id && !isSuperAdmin(user) && targetVisibleInLive(user));
  if (!eligibleUsers.length) {
    return;
  }

  const billingPeriod = stamp.slice(0, 7);
  const userIds = Array.from(new Set(eligibleUsers.map((user) => user.id)));
  const existingCredits = await portal.creditLedger
    .find(
      {
        userId: { $in: userIds },
        creditType: "post",
    ...
    .toArray();
  const creditedUserIds = new Set(existingCredits.map((credit) => credit.userId));
  const refillUsers = eligibleUsers.filter((user) => !creditedUserIds.has(user.id));
  if (!refillUsers.length) {
    return;
  }

  const refillUserIds = refillUsers.map((user) => user.id);
  const postTotals = await portal.creditLedger
    .aggregate([
      { $match: { userId: { $in: refillUserIds }, creditType: { $in: ["post", "gift"] } } },
      { $group: { _id: "$userId", totalPostCredit: { $sum: "$amount" } } },
    ])
    .toArray();
  const postCreditByUser = new Map(postTotals.map((total) => [total._id, cleanPostCreditAmount(total.totalPostCredit)]));

  await portal.creditLedger.insertMany(
    refillUsers.map((user) => {
      const currentBalance = postCreditByUser.get(user.id) || 0;
...
async function creditBalancesForUser(portal, user) {
  const balancesByUser = await creditBalancesForUsers(portal, [user]);
  return balancesByUser.get(user?.id || "") || {
    moneyCreditBalance: 0,
    postCreditBalance: 0,
    postingCreditBalance: 0,
  };
}

async function creditBalancesForUsers(portal, users) {
  const targetUsers = (Array.isArray(users) ? users : []).filter((user) => user?.id && !isSuperAdmin(user));
  if (!targetUsers.length) {
    return new Map();
  }

  await ensureMonthlyPostCreditForUsers(portal, targetUsers);

  const userIds = Array.from(new Set(targetUsers.map((user) => user.id)));
  const clientIds = targetUsers.filter((user) => isClientRole(user.role)).map((user) => user.id);

  const [depositTotals, paymentTotals, moneyLedgerTotals, postLedgerTotals] = await Promise.all([
    clientIds.length
      ? portal.deposits
          .aggregate([
            { $match: { clientId: { $in: clientIds }, status: "paid" } },
            { $group: { _id: "$clientId", totalCredits: { $sum: "$creditAmount" } } },
          ])
          .toArray()
      : [],
    clientIds.length
      ? portal.payments
          .aggregate([
            { $match: { clientId: { $in: clientIds }, status: { $in: ["paid", "processing"] } } },
            { $group: { _id: "$clientId", totalSpent: { $sum: "$amount" } } },
          ])
          .toArray()
      : [],
    portal.creditLedger
      .aggregate([
        { $match: { userId: { $in: userIds }, creditType: "money" } },
        { $group: { _id: "$userId", totalMoney: { $sum: "$amount" }, heldMoney: { $sum: { $cond: [{ $and: [{ $gt: ["$withdrawalAvailableAt", now()] }, { $gt: ["$amount", 0] }, { $eq: ["$source", "client_release"] }] }, "$amount", 0] } } } },
      ])
      .toArray(),
    portal.creditLedger
      .aggregate([
        { $match: { userId: { $in: userIds }, creditType: { $in: ["post", "gift"] } } },
        { $group: { _id: "$userId", totalPostCredit: { $sum: "$amount" } } },
      ])
      .toArray(),
  ]);

  const depositsByClient = new Map(depositTotals.map((total) => [total._id, cleanNumber(total.totalCredits)]));
  const paymentsByClient = new Map(paymentTotals.map((total) => [total._id, cleanNumber(total.totalSpent)]));
  const moneyByUser = new Map(moneyLedgerTotals.map((total) => [total._id, cleanSignedNumber(total.totalMoney)]));
  const postByUser = new Map(postLedgerTotals.map((total) => [total._id, cleanPostCreditAmount(total.totalPostCredit)]));
  const heldByUser = new Map(moneyLedgerTotals.map((total) => [total._id, cleanNumber(total.heldMoney)]));

  return new Map(
    targetUsers.map((user) => {
      const moneyAdjustments = moneyByUser.get(user.id) || 0;
      const moneyCreditBalance = isClientRole(user.role)
        ? roundSignedMoney((depositsByClient.get(user.id) || 0) - (paymentsByClient.get(user.id) || 0) + moneyAdjustments)
        : Math.max(0, roundSignedMoney(moneyAdjustments));
      const postCreditBalanceAmount = Math.max(0, cleanPostCreditAmount(postByUser.get(user.id) || 0));

      return [
        user.id,
        {
          moneyCreditBalance,
          heldMoneyCreditBalance: Math.max(0, Math.min(moneyCreditBalance, heldByUser.get(user.id) || 0)),
          withdrawableBalance: Math.max(0, roundMoney(moneyCreditBalance - (heldByUser.get(user.id) || 0))),
...
async function recordBidderPaymentCredit(portal, { paymentId, bidder, client, amount, memo = "", stamp = now() }) {
  ...
  const creditResult = await portal.creditLedger.updateOne(
    {
      userId: bidder.id,
      creditType: "money",
    ...
    },
    {
      $setOnInsert: {
        id: createId("credit"),
        createdAt: stamp,
        withdrawalAvailableAt: withdrawalAvailableAt(stamp),
      },
      $set: {
        amount: creditAmount,
        relatedPostId: "",
  ...
  if (creditResult.upsertedCount) {
    try {
      const payment = await portal.payments.findOne({ id: paymentId }, { projection: { _id: 0, periodStart: 1, periodEnd: 1 } });
      const email = paymentReceivedEmail({
        name: bidder.name, clientName: client?.name, amount: creditAmount, paymentId,
        periodStart: payment?.periodStart, periodEnd: payment?.periodEnd,
        availableAt: withdrawalAvailableAt(stamp), baseUrl: appBaseUrl(),
      });
      await sendPortalEmail(portal, { to: bidder.email, ...email });
      await portal.payments.updateOne({ id: paymentId }, { $set: { receiptEmailStatus: "sent", receiptEmailSentAt: now() } });
    } catch {
      // Email delivery must not turn a successful credit into a failed payment response.
      await portal.payments.updateOne({ id: paymentId }, { $set: { receiptEmailStatus: "failed" } }).catch(() => {});
    }
  }
}
...
function moneyLabel(amount) {
  return `$${roundSignedMoney(amount).toFixed(2)}`;
}
...
function accountStatusNotification(status, previousStatus) {
  const normalizedStatus = normalizeUserStatus(status);
  const previousNormalizedStatus = normalizeUserStatus(previousStatus, "");
  if (normalizedStatus === "active" && previousNormalizedStatus === "pending_review") {
    return {
      title: "Account approved",
      body: "Super admin approved your account. You can now use the bidder portal tools.",
    };
  }
  if (normalizedStatus === "active") {
    return {
      title: "Account activated",
      body: "Super admin activated your account. You can use the bidder portal tools again.",
    };
  }
  if (normalizedStatus === "pending_review") {
    return {
      title: "Account moved to pending review",
      body: "Super admin moved your account to pending review. Access will open again after approval.",
    };
  }
  if (normalizedStatus === "temporarily_restricted") {
    return {
      title: "Account temporarily restricted",
      body: "Super admin temporarily restricted your account. Contact support if you need help.",
    };
  }
  if (normalizedStatus === "suspended") {
    return {
      title: "Account suspended",
      body: "Super admin suspended your account. Contact support for help.",
    };
  }
  if (normalizedStatus === "closed") {
    return {
      title: "Account closed",
      body: "Super admin closed your account. You can delete the account if you want to register this email again later.",
    };
  }
  return {
    title: "Account status updated",
    body: `Super admin changed your account status to ${accountStatusLabel(normalizedStatus)}.`,
  };
}
...
async function notifyAssignedClientForWorkLog(portal, { bidder, workLogId, contractId = "", clientId = "", type, title, body }) {
  const assignedClientId = cleanText(clientId) || cleanText(bidder?.assignedAdminId);
  if (!assignedClientId) {
    return;
  }
  ...
  await createUserNotification(portal, {
    ...
    relatedContractId: cleanText(contractId),
...
async function getPortalDataForUser(currentUser, sessionToken = "") {
  ...
  const [paymentMethodsPage, workLogsPage, paymentsPage, escrowsPage, depositsPage, contractsPage, postsPage, bidProfilesPage, disputesPage, notificationsPage, chatContacts, chatMessagesPage, supportContacts, supportMessagesPage] = await Promise.all([
    ...
    fetchPortalPage(
      ...
      { createdAt: -1 },
      "chatMessages",
      { reverseItems: true, projection: CHAT_ATTACHMENT_META_PROJECTION }
    ),
  ...
  return {
    ...
    chatContacts,
    chatMessages: stripChatAttachmentDataUrls(chatMessagesPage.items),
    supportContacts,
...
async function assertCanReleasePaymentForClient(portal, client, target) {
  if (canManagePaymentUser(client, target)) {
    return;
  }

  if (!isClientRole(client?.role) || !isPortalWorker(target)) {
    throw new Error("You can only manage payments for assigned bidders.");
  }

  const relatedContract = await portal.contracts.findOne(
    {
      clientId: client.id,
      workerId: target.id,
  ...
  if (!relatedContract) {
    throw new Error("You can only manage payments for bidders with your contracts.");
  }
}
...
export async function requestPasswordReset(emailInput) {
  ...
  await sendPortalEmail(portal, {
    to: email,
    ...accountEmail({ kind: "password_reset", name: user.name, actionUrl: resetLink, expiresInMs: RESET_TOKEN_MS }),
  });
...
async function sendEmailVerificationToUser(portal, target) {
  ...
  await sendPortalEmail(portal, {
    to: target.email,
    ...accountEmail({ kind: "email_verification", name: target.name, actionUrl: verifyLink, expiresInMs: VERIFY_TOKEN_MS }),
  });
...
export async function updateUserAsAdmin(adminEmail, payload) {
  ...
  const previousStatus = normalizeUserStatus(target.status);
  const safeStatus = normalizeUserStatus(payload.status, target.status);
  const statusChanged = previousStatus !== safeStatus;
  ...
  if (statusChanged && !isUserActive(safeStatus)) {
    await portal.sessions.deleteMany({ userId: target.id });
  }
  if (statusChanged) {
    const notification = accountStatusNotification(safeStatus, previousStatus);
    await createUserNotification(portal, {
...
export async function createDispute(email, payload) {
  const currentUser = await getSessionUser(email, payload.sessionToken);
  if (!isUserActive(currentUser) || (!isClientRole(currentUser.role) && !isPortalWorker(currentUser))) {
    throw new Error("Only active clients and bidders can open disputes.");
  }
  ...
  const disputedPayment = paymentId ? await portal.payments.findOne({
    id: paymentId,
    ...(isClientRole(currentUser.role) ? { clientId: currentUser.id } : { userId: currentUser.id }),
  }, { projection: { _id: 0 } }) : null;
  if (paymentId && (!disputedPayment || !disputedPayment.clientId || disputedPayment.paymentType === "withdrawal")) {
    throw new Error("Select a valid work payment for this dispute.");
  }
  const contract = contractId
    ? await portal.contracts.findOne({ id: contractId, ...(isClientRole(currentUser.role) ? { clientId: currentUser.id } : { workerId: currentUser.id }) }, { projection: { _id: 0 } })
    : null;
  if ((contractId && !contract) || (!isClientRole(currentUser.role) && !contract && !disputedPayment)) {
    throw new Error("Select your contract for this dispute.");
  }
  const clientId = contract?.clientId || disputedPayment?.clientId || currentUser.id;
  const targetUserId = contract?.workerId || disputedPayment?.userId || cleanText(payload.targetUserId);
  if (targetUserId) {
    const target = await getUserById(targetUserId);
    const previousContract = contract || await portal.contracts.findOne({ clientId, workerId: targetUserId });
    if (!target || !isPortalWorker(target) || (!previousContract && !disputedPayment && target.assignedAdminId !== currentUser.id) || !targetVisibleInLive(target)) {
      throw new Error("Select an assigned bidder for this dispute.");
    }
  }

  if (paymentId) {
    const payment = await portal.payments.findOne({ id: paymentId, clientId, ...(targetUserId ? { userId: targetUserId } : {}) }, { projection: { _id: 1 } });
    if (!payment) {
  ...
  await Promise.all([
    ...
    targetUserId
      ? createUserNotification(portal, {
        recipientUserId: currentUser.id === targetUserId ? clientId : targetUserId,
        type: "dispute_created",
        title: "New dispute opened",
        body: `${currentUser.name || "Client"} opened a dispute: ${subject}.`,
        relatedDisputeId: disputeRecord.id,
...
export async function adjustCreditAsSuperAdmin(adminEmail, payload) {
  ...
  if (creditType === "money" && isClientRole(target.role)) await notifyClientWorkShortfalls(portal, target.id);
...
export async function createContract(email, payload) {
  ...
  const workDays = normalizeContractWorkDays(payload.workDays);
  const portal = collections(await getDb());
...
export async function updateContract(email, payload) {
  ...
  const workDays = normalizeContractWorkDays(payload.workDays ?? contract.workDays);
  const stamp = now();
  const updates = {
    title,
    criteria,
...
async function notifyClientWorkShortfalls(portal, clientId) {
  const workerIds = await portal.contracts.distinct("workerId", { clientId });
  for (const workerId of workerIds) await notifyWorkCreditShortfall(portal, clientId, workerId);
}

async function notifyWorkCreditShortfall(portal, clientId, bidderId) {
  const [client, bidder, contracts, payments] = await Promise.all([
    getUserById(clientId), getUserById(bidderId),
    portal.contracts.find({ clientId, workerId: bidderId }, { projection: { _id: 0, id: 1 } }).toArray(),
    portal.payments.find({ clientId, userId: bidderId, paymentType: { $ne: WITHDRAWAL_PAYMENT_TYPE }, status: { $in: ["paid", "processing"] } }, { projection: { _id: 0, periodStart: 1, periodEnd: 1 } }).toArray(),
  ]);
  if (!client || !bidder || !contracts.length) return;
  const logs = await portal.workLogs.find({ userId: bidderId, contractId: { $in: contracts.map((contract) => contract.id) } }, { projection: { _id: 0 } }).toArray();
  const unpaid = logs.filter((log) => !payments.some((payment) => payment.periodStart <= log.workDate && payment.periodEnd >= log.workDate));
  const owed = roundMoney(unpaid.reduce((total, log) => total + cleanNumber(log.appliedJobs) * cleanNumber(bidder.ratePerApplication) + cleanNumber(log.interviewsScheduled) * cleanNumber(bidder.bonusPerInterview), 0));
  const balance = await clientCreditBalance(portal, clientId);
  const notificationId = `work-credit-shortfall:${clientId}:${bidderId}`;
  if (owed <= balance) {
    await portal.notifications.deleteOne({ id: notificationId });
    return;
  }
  const existing = await portal.notifications.findOne({ id: notificationId });
  if (existing?.amount === owed && existing?.creditAmount === balance) return;
  const stamp = now();
  await portal.notifications.updateOne({ id: notificationId }, { $set: {
    recipientUserId: bidderId, recipientRole: "", type: "billing_credit_shortfall",
    title: "Client credit is below your unpaid work",
    body: `${client.name || "Your client"} has ${moneyLabel(balance)} available credit, below your ${moneyLabel(owed)} of submitted unpaid work. Shortfall: ${moneyLabel(owed - balance)}. Payment still requires approved work.`,
    relatedContractId: unpaid[0]?.contractId || "", clientId, actorUserId: clientId,
    amount: owed, creditAmount: balance, readAt: "", createdAt: stamp, updatedAt: stamp,
  } }, { upsert: true });
}
...
export async function saveWorkLog(email, payload) {
  ...
  if (!parseIsoDate(workDate)) {
    throw new Error("Select a valid work date.");
  }
  ...
  if (workLogId) {
    ...
    const selectedContract = await activeContractForWorkLog(
      portal,
      currentUser.id,
      contractId || existingLog.contractId,
      workDate
    );
    ...
    await notifyWorkCreditShortfall(portal, selectedContract.clientId, currentUser.id);
  ...
  const selectedContract = await activeContractForWorkLog(portal, currentUser.id, contractId, workDate);
...
export async function reviewWorkLog(email, payload) {
  ...
  await notifyWorkCreditShortfall(portal, currentUser.id, bidder.id);
  return getPortalDataForUser(currentUser, payload.sessionToken);
}

export async function releasePaymentAsClient(clientEmail, payload) {
  const actor = await getSessionUser(clientEmail, payload.sessionToken);
  if (!isUserActive(actor) || (!isClientRole(actor.role) && !isSuperAdmin(actor))) {
    throw new Error("Only active clients and super admins can release bidder payments.");
  }
  const client = isSuperAdmin(actor) ? await getUserById(cleanText(payload.clientId)) : actor;
  if (!client || !isClientRole(client.role) || !targetVisibleInLive(client)) throw new Error("Select a valid client for payment.");
  ...
  const portal = collections(await getDb());
  await assertCanReleasePaymentForClient(portal, client, target);
  ...
  const adminOverdueAmount = isSuperAdmin(actor)
    ? overduePaymentAmount(sourcePayment, client.id, target.id, dateToIso(new Date()))
    : null;
  if (isSuperAdmin(actor) && (periodStart !== sourcePayment.periodStart || periodEnd !== sourcePayment.periodEnd)) {
    throw new Error("Use the overdue payment's recorded work period.");
  }
  ...
  const clientContracts = stripMongoIds(
    await portal.contracts
      .find({ clientId: client.id, workerId: target.id, status: { $in: ["active", "ended"] } }, { projection: { _id: 0, id: 1 } })
      .toArray()
  );
  const clientContractIds = clientContracts.map((contract) => contract.id).filter(Boolean);
  if (!clientContractIds.length) {
    throw new Error("Select a bidder with your active or ended contract.");
  }
  const logs = await portal.workLogs
    .find(
      approvedClientWorkFilter(client.id, target.id, clientContractIds, periodStart, periodEnd),
      { projection: { _id: 0 } }
    )
    .toArray();
  ...
  const paymentPortion = cleanText(payload.paymentPortion).toLowerCase() === "half" ? "half" : "full";
  if (requestedBaseAmount != null && requestedBaseAmount < 0) {
    throw new Error("Payment release amount is required.");
  }
  if (!isSuperAdmin(actor) && requestedBaseAmount != null && requestedBaseAmount > calculatedBaseAmount) {
    throw new Error(`The requested amount exceeds the approved work total of ${moneyLabel(calculatedBaseAmount)} for this client and period. Refresh the billing page and check the selected dates.`);
  }
  const baseAmount = adminOverdueAmount ?? requestedBaseAmount ?? calculatedBaseAmount;
  const tipAmount = isSuperAdmin(actor) ? 0 : roundMoney(payload.tipAmount);
  ...
  if (creditBalance < amount && !isSuperAdmin(actor)) {
    await notifyWorkCreditShortfall(portal, client.id, target.id);
  ...
  const paymentRecord = {
    ...
    amount,
    baseAmount,
    paymentPortion: isSuperAdmin(actor) ? "full" : paymentPortion,
    tipAmount,
  ...
  if (sourcePayment) {
    const released = await portal.payments.updateOne({ id: sourcePayment.id, status: "scheduled" }, { $set: paymentRecord });
    if (!released.modifiedCount) throw new Error("This payment has already been released or changed. Refresh the payment list.");
  } else {
  ...
  await notifyClientWorkShortfalls(portal, client.id);
  if (isSuperAdmin(actor)) await createUserNotification(portal, {
    recipientUserId: client.id, type: "billing_payment_released", title: "Super admin paid approved work",
    body: `${actor.name || "Super admin"} paid the overdue amount of ${moneyLabel(amount)} to ${target.name || "your bidder"} from your credit. Your balance is now ${moneyLabel(creditBalance - amount)}.`,
    relatedPaymentId: paymentId, actorUserId: actor.id, amount,
  });
  return getPortalDataForUser(actor, payload.sessionToken);
}
...
export async function requestWithdrawal(email, payload) {
  ...
  const balances = await creditBalancesForUser(portal, currentUser);
  if ((balances.withdrawableBalance ?? 0) < amount) {
    throw new Error("This amount is not available to withdraw. Released payments are held for 3 business days (Monday-Friday).");
  }
...
export async function addChatMessage(email, payload) {
  ...
  assertSafeInboxText(body, ...attachments.map((attachment) => attachment.name));
...
export async function addChatAlertMessage(email, payload) {
  const currentUser = await getSessionUser(email, payload.sessionToken);
  if (!isUserActive(currentUser)) {
    throw new Error("Only active super admins can send inbox alerts.");
  }
  if (!isSuperAdmin(currentUser)) {
    throw new Error("Only super admins can send inbox alerts.");
  }
  ...
  assertSafeInboxText(body);

  const portal = collections(await getDb());
  const conversationId = cleanText(payload.conversationId);
  const participantIds = chatParticipantIdsFromConversationId(conversationId);
  const recipientId = cleanText(payload.recipientId);
  if (!conversationId || participantIds.length < 2 || !recipientId) {
    throw new Error("Select a valid monitored inbox conversation.");
  }
  if (!participantIds.includes(recipientId)) {
    throw new Error("Alert recipient must be in this inbox conversation.");
  }

  const [existingMessage, recipient] = await Promise.all([
    portal.chatMessages.findOne(
      {
        conversationId,
        ...directMessageChannelFilter(),
      },
      { projection: { _id: 0, id: 1, relatedPostId: 1 } }
    ),
  ...
  if (!recipient || isSuperAdmin(recipient) || !isUserActive(recipient) || !targetVisibleInLive(recipient)) {
    throw new Error("Select a valid alert recipient.");
  }
  ...
  await portal.chatMessages.insertOne({
    ...
    body: body.slice(0, 1000),
    attachments: [],
    relatedPostId: existingMessage.relatedPostId || "",
    authorTimeZone: cleanText(payload.authorTimeZone).slice(0, 80),
    createdAt: stamp,
    updatedAt: stamp,
    editedAt: "",
