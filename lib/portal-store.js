...
const PUBLIC_USER_PROJECTION = { _id: 0, passwordHash: 0, passwordUpdatedAt: 0 };
...
function publicUser(user) {
  ...
  delete record.passwordUpdatedAt;
  return record;
}
...
export async function signUp(emailInput, nameInput, passwordInput) {
  ...
  return getPortalDataForUser(user, sessionToken);
}
...
async function getPortalDataForUser(currentUser, sessionToken = "") {
  ...
  const users = isAdmin
    ? stripMongoIds(
        await portal.users
          .find(userVisibilityFilter, { projection: PUBLIC_USER_PROJECTION })
