  ...
  return new Map(
    targetUsers.map((user) => {
      ...
      const moneyCreditBalance = isClientRole(user.role)
        ? roundMoney((depositsByClient.get(user.id) || 0) - (paymentsByClient.get(user.id) || 0) + moneyAdjustments)
      ...
      return [
        ...
        {
          moneyCreditBalance,
          heldMoneyCreditBalance: Math.min(moneyCreditBalance, heldByUser.get(user.id) || 0),
...
export async function releasePaymentAsClient(clientEmail, payload) {
  ...
  if (isSuperAdmin(actor)) await createUserNotification(portal, {
    ...
    body: `${actor.name || "Super admin"} released ${moneyLabel(amount)} from your credit to ${target.name || "your bidder"}.`,
