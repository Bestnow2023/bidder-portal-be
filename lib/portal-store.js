...
export async function createDispute(email, payload) {
  ...
  const contract = contractId
    ...
    : null;
  if ((contractId && !contract) || (!isClientRole(currentUser.role) && !contract)) {
    throw new Error("Select your contract for this dispute.");
  }
  const clientId = contract?.clientId || currentUser.id;
  const targetUserId = contract?.workerId || cleanText(payload.targetUserId);
  if (targetUserId) {
    const target = await getUserById(targetUserId);
    const previousContract = contract || await portal.contracts.findOne({ clientId, workerId: targetUserId });
    if (!target || !isPortalWorker(target) || (!previousContract && target.assignedAdminId !== currentUser.id) || !targetVisibleInLive(target)) {
