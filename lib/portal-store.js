...
async function creditBalancesForUser(portal, user) {
  ...
  return {
    ...
    postingCreditBalance: roundMoney(moneyCreditBalance + postCreditBalanceAmount),
  };
}
...
async function getPortalDataForUser(currentUser, sessionToken = "") {
  ...
  const [paymentMethods, workLogs, payments, escrows, deposits, contracts, posts, bidProfiles, notifications, chatContacts, chatMessages] = await Promise.all([
