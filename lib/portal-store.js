...
async function clientCreditBalance(portal, clientId, options = {}) {
  ...
  return roundMoney(cleanNumber(depositTotals[0]?.totalCredits) - cleanNumber(paymentTotals[0]?.totalSpent));
}
...
async function getPortalDataForUser(currentUser, sessionToken = "") {
  ...
  const [paymentMethods, workLogs, payments, escrows, deposits, notifications, chatContacts, chatMessages] = await Promise.all([
