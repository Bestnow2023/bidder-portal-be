...
async function releasedPaymentOverlapsPeriod(portal, userId, periodStart, periodEnd) {
  return Boolean(
    await portal.payments.findOne(
      {
        userId,
...
async function ensureSignupGiftCredit(portal, userId, stamp = now()) {
  if (!userId) {
    return;
  }
...
async function seedDemoData(portal) {
  ...
  await Promise.all([
    ensureSignupGiftCredit(portal, "user_client", stamp),
    ensureSignupGiftCredit(portal, "user_maya", stamp),
    ensureSignupGiftCredit(portal, "user_pending", stamp),
    ...
    portal.creditLedger.updateOne(
      { userId: "user_client", source: "post_fee", relatedPostId: "post_demo_client", creditType: "gift" },
      {
        $setOnInsert: {
    ...
    portal.creditLedger.updateOne(
      { userId: "user_maya", source: "post_fee", relatedPostId: "post_demo_bidder", creditType: "gift" },
      {
        $setOnInsert: {
    ...
    portal.chatMessages.updateOne(
      { id: "chat_welcome" },
      {
        $setOnInsert: {
          ...
          conversationId: chatConversationId("user_admin", "user_maya"),
...
async function createUser(email, nameInput, role, status, password, assignedAdminId = "") {
  ...
  if (result.upsertedCount) {
    await ensureSignupGiftCredit(portal, user.id, stamp);
  }
...
async function giftCreditBalance(portal, userId) {
  const giftTotals = await portal.creditLedger
    .aggregate([
      { $match: { userId, creditType: "gift" } },
      { $group: { _id: "$userId", totalGift: { $sum: "$amount" } } },
    ])
    .toArray();

  return Math.max(0, roundSignedMoney(giftTotals[0]?.totalGift));
}
...
async function creditBalancesForUser(portal, user) {
  const [moneyCreditBalance, giftCreditBalanceAmount] = await Promise.all([
    moneyCreditBalanceForUser(portal, user),
    giftCreditBalance(portal, user?.id || ""),
  ]);

  return {
    moneyCreditBalance,
    giftCreditBalance: giftCreditBalanceAmount,
    postingCreditBalance: roundMoney(moneyCreditBalance + giftCreditBalanceAmount),
  };
}
...
async function spendPostingCredit(portal, user, postId, stamp) {
  ...
  if (balances.postingCreditBalance < POST_CREDIT_COST) {
    throw new Error("You need at least $1 credit to publish a post.");
  }

  const giftCreditUsed = Math.min(POST_CREDIT_COST, balances.giftCreditBalance);
  const moneyCreditUsed = roundMoney(POST_CREDIT_COST - giftCreditUsed);
  const ledgerEntries = [];
  if (giftCreditUsed > 0) {
    ledgerEntries.push({
      id: createId("credit"),
  ...
  if (ledgerEntries.length) {
    await portal.creditLedger.insertMany(ledgerEntries);
  }

  return { giftCreditUsed, moneyCreditUsed };
}
...
async function getPortalDataForUser(currentUser, sessionToken = "") {
  ...
  const notificationFilter = isSuperAdmin(safeCurrentUser)
    ? { recipientRole: "super_admin" }
    : { recipientRole: "__none__" };

  const [paymentMethods, workLogs, payments, escrows, deposits, contracts, posts, notifications, chatContacts, chatMessages] = await Promise.all([
...
export async function addManualCreditAsSuperAdmin(adminEmail, payload) {
  const admin = await getSessionUser(adminEmail, payload.sessionToken);
  if (!isSuperAdmin(admin)) {
    throw new Error("Only super admins can add manual client credits.");
  }

  const portal = collections(await getDb());
  const targetClientId = cleanText(payload.clientId);
  const targetClient = await getUserById(targetClientId);
  if (!targetClient || !isClientRole(targetClient.role) || !targetVisibleInLive(targetClient)) {
    throw new Error("Select a valid client for manual credit.");
  }
  ...
  await portal.deposits.insertOne({
    ...
    clientId: targetClient.id,
    provider: "manual",
    orderId: `manual_${randomUUID().replace(/-/g, "")}`,
    invoiceUuid: "",
    amount: creditAmount,
    feeAmount: 0,
    creditAmount,
    ...
    network: "",
    status: "paid",
    providerStatus: "manual_credit",
    paymentUrl: cleanText(payload.referenceLink),
    paymentAmountUsd: creditAmount,
    merchantAmount: creditAmount,
    txid: "",
    memo: cleanText(payload.memo),
    rawProvider: {
...
export async function createPost(email, payload) {
  ...
  const portal = collections(await getDb());
  const { giftCreditUsed, moneyCreditUsed } = await spendPostingCredit(portal, currentUser, postId, stamp);
...
export async function releasePaymentAsClient(clientEmail, payload) {
  ...
  const portal = collections(await getDb());
  if (await releasedPaymentOverlapsPeriod(portal, target.id, periodStart, periodEnd)) {
  ...
  const logs = await portal.workLogs
    .find({ userId: target.id, workDate: { $gte: periodStart, $lte: periodEnd } }, { projection: { _id: 0 } })
    .toArray();
  ...
  const nextPaymentDate = schedule.frequency && schedule.weekday
    ? await nextOpenPaymentDate(portal, target.id, schedule.frequency, schedule.weekday, target.nextPaymentDate || periodEnd)
    : periodEnd;
...
export async function addChatMessage(email, payload) {
  ...
  if (recipient.allowDirectMessages === false && !isSuperAdmin(currentUser)) {
